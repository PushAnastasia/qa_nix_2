...
public abstract class AbstractAuthorDao {
    ...
    protected Author findAuthorById(String id) {
        return findAllAuthors().stream().filter(author -> author.getId().equals(id)).findFirst().get();
    }

    protected List<Author> findAllAuthors() {
        return authors.stream().filter(Author::getIsVisible).collect(Collectors.toList());
    }
